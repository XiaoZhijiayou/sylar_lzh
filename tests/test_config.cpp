...
void print_yaml(const YAML::Node node, int level) {
  if (node.IsScalar()) {
    SYLAR_LOG_INFO(SYLAR_LOG_ROOT())
        << std::string(level * 4, ' ') << node.Scalar() << "-" << node.Tag();
  } else if (node.IsNull()) {
...
void test_yaml() {
  YAML::Node root =
      YAML::LoadFile("/home/li/project/sylar/bin/conf/test.yml");
  print_yaml(root, 0);
...
#define XX_M(g_var, name, prefix)                                              \
  ...
  XX(g_int_vec_value_config, g_int_vec, after);
...
void test_log() {
  ...
  YAML::Node root =
      YAML::LoadFile("/home/li/project/sylar/bin/conf/log.yml");
  sylar::Config::LoadFromYaml(root);
  std::cout << "===========================================" << std::endl;
  std::cout << sylar::LoggerMgr::GetInstance()->toYamlString() << std::endl;
  std::cout << "===========================================" << std::endl;
