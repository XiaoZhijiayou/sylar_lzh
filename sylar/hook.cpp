...
int connect(int socket, const struct sockaddr* address,
  ...
  return connect_with_timeout(socket,address,address_len,sylar::s_connect_timeout);
}

int accept(int socket, struct sockaddr* address,
                  socklen_t* address_len){
  int fd = do_io(socket,accept_f,"accept",sylar::IOManager::READ,SO_RCVTIMEO,address,address_len);
  if(fd >= 0){
    sylar::FdMgr::GetInstance()->get(fd, true);
  }
  return fd;
...
ssize_t readv(int fildes, const struct iovec* iov, int iovcnt){
  return do_io(fildes,readv_f,"readv",sylar::IOManager::READ,SO_RCVTIMEO,iov,iovcnt);
}


ssize_t recv(int socket, void* buffer, size_t length, int flags){
    return do_io(socket,recv_f,"recv",sylar::IOManager::READ,SO_RCVTIMEO,buffer,length,flags);
}
...
ssize_t recvfrom(int socket, void* buffer, size_t length,
                                ...
                                socklen_t* address_len){
    return do_io(socket,recvfrom_f,"recvfrom",sylar::IOManager::READ,SO_RCVTIMEO,
                 buffer,length,flags,address,address_len);
}


ssize_t recvmsg(int socket, struct msghdr* message, int flags){
  return do_io(socket, recvmsg_f,"recvmsg",sylar::IOManager::READ,SO_RCVTIMEO
               ,message,flags);
}
...
ssize_t send(int socket, const void* buffer, size_t length,
                            int flags){
    return do_io(socket,send_f,"send",sylar::IOManager::WRITE,SO_SNDTIMEO,buffer,length,flags);
}
...
ssize_t sendto(int socket, const void* message, size_t length,
                              ...
                              socklen_t dest_len){
    return do_io(socket,sendto_f,"send_to",sylar::IOManager::WRITE,SO_SNDTIMEO,message,length,flags,dest_addr,dest_len);
}


ssize_t sendmsg(int socket, const struct msghdr* message,
                               int flags){
  return do_io(socket,sendmsg_f,"sendmsg",sylar::IOManager::WRITE,SO_SNDTIMEO,message,flags);
}
