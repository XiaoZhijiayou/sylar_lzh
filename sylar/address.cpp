 ...
 * 例如 T = uint32_t, bits = 4 ： 00010000 00000000 00000000 00000000
 * -1 之后：00001111 11111111 11111111 11111111
 * */
template <class T>
...
IPv4Address::ptr IPv4Address::Create(const char* address, uint16_t port) {
  ...
  if (result <= 0) {
    SYLAR_LOG_ERROR(g_logger)
        ...
        << "erron" << errno << "errstr = " << strerror(errno);
    return nullptr;
  }
  return rt;
}
...
std::ostream& IPv6Address::insert(std::ostream& os) const {
  os << "[";
  /* 这里的 m_addr.sin6_addr.s6_addr 是一个 uint8_t[16] 类型的数组
     * （IPv6 地址是 128 位 = 16 字节），
     * 每个元素是 8 位的无符号整数（uint8_t）*/
...
UnixAddress::UnixAddress(const std::string& path) {
  ...
  /*offsetof 是一个 C/C++ 宏函数，用来计算结构体中某个字段相对于结构体首地址的字节偏移量*/
