...
int Socket::recvFrom(void* buffers, size_t length, Address::ptr from,
                     int flags) {
  if (isConnected()) {
...
std::ostream& Socket::dump(std::ostream& os) const {
  os << "[Socket sock =" << m_sock << "is_connected=" << m_isConnect
...
void Socket::newSock() {
  ...
  if (SYLAR_LIKELY(m_sock != -1)) {
    ...
    SYLAR_LOG_ERROR(g_logger)
        << "sock(" << m_family << "," << m_type << "," << m_protocol
