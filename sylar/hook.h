...
extern "C" {
  ...
  /// 服务器端与客户端建立连接
  typedef int (*accept_fun)(int socket, struct sockaddr *address,socklen_t *address_len);
  ...
  typedef ssize_t (*send_fun)(int socket, const void *buffer, size_t length, int flags);
  ...
  typedef ssize_t (*sendmsg_fun)(int socket, const struct msghdr *message, int flags);
