...
class HttpResponse {
 public:
  ...
  /**
   * @brief 为响应添加cookie
   * @param[in] key cookie的key值
   * @param[in] val cookie的val值
   * @param[in] expired 过期时间
   * @param[in] path cookie的影响路径
   * @param[in] domain cookie作用的域
   * @param[in] secure 安全标志
   * */
//  void setCookie(const std::string& key, const std::string& val,
