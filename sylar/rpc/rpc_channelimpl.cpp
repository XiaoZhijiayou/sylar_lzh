...
void RpcChannelImpl::CallMethod(
    ...
    ZkClientRPC zkCli;
    zkCli.start();
    SYLAR_LOG_DEBUG(g_rpclogger) << "创建 zk client";
    ...
    if(!client->isConnected()) {
        SYLAR_LOG_INFO(g_rpclogger) << "connection rpc server failed";
        return;
    }

    client->getCond().wait(client->getMutex());

    SYLAR_LOG_INFO(g_rpclogger) << "close connection by RpcProvider";
    ...
    SYLAR_LOG_INFO(g_rpclogger) << "接受结束";
}

void RpcChannelImpl::onConnection(const Socket::ptr& conn) {
    if(conn->isConnected()) {
        SYLAR_LOG_INFO(g_rpclogger) << "Connection Up";
        // 发送RPC请求
        conn->send(m_package.c_str(), m_package.size());
        SYLAR_LOG_INFO(g_rpclogger) << "send package to RpcProvider: " << m_package;
    } else {
        SYLAR_LOG_INFO(g_rpclogger) << "Connection Down";
        conn->close();
    }
}
...
void RpcChannelImpl::onMessage(const Socket::ptr& conn) {
    // 接受RPC应答
    SYLAR_LOG_INFO(g_rpclogger) << "jieshou";
    ...
    SYLAR_LOG_INFO(g_rpclogger) << "receive response from RpcProvider: " << m_result;
    SYLAR_LOG_INFO(g_rpclogger) <<  m_result << " " << m_result.size();
    // conn->cancelAll();
    conn->close();
}
