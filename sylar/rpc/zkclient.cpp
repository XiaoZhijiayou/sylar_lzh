...
void g_watcher(zhandle_t* handler, int type, int state,
    ...
    SYLAR_LOG_FATAL(g_rpclogger) << "触发 g_watch 连接回调";
    if (type == ZOO_SESSION_EVENT) {
        SYLAR_LOG_FATAL(g_rpclogger) << "会话事件";
        if (state == ZOO_CONNECTED_STATE) {
            SYLAR_LOG_FATAL(g_rpclogger) << "链接事件";
...
void ZkClientRPC::start() {
    ...
    if (nullptr == m_zkHandler) {
        SYLAR_LOG_FATAL(g_rpclogger) << "failed to initialize zookeeper";
        exit(EXIT_FAILURE);
    }
    ...
    SYLAR_LOG_FATAL(g_rpclogger) << "zk句柄初始化成功";

    FiberMutex::ptr mutex = std::make_shared<FiberMutex>();
    // FiberCondition* cond = new FiberCondition;
    // mutex->lock();
    SYLAR_LOG_FATAL(g_rpclogger) << "lock成功";
    zoo_set_context(m_zkHandler, &sem);
    SYLAR_LOG_FATAL(g_rpclogger) << "传递上下文成功";
    // 阻塞直到zkServer响应链接创建成功
    // cond->wait(*mutex);
    // sleep(5);
    sem_wait(&sem);
    SYLAR_LOG_FATAL(g_rpclogger) << "链接创建成功";
    // mutex->unlock();
    // delete(cond);
    SYLAR_LOG_INFO(g_rpclogger) << "initialize zookeeper successfully";
}
...
void ZkClientRPC::create(const std::string& path, const std::string& data, int flags) {
    ...
    if (flag == ZNONODE) {
        ...
        if (flag == ZOK) {
            SYLAR_LOG_INFO(g_rpclogger) << "create znode: " << path.c_str();
        } else {
            SYLAR_LOG_FATAL(g_rpclogger) << "failed to create znode: " << path.c_str();
            exit(EXIT_FAILURE);
        }
    }
}
...
std::string ZkClientRPC::getData(const std::string& path) {
    ...
    if (flag != ZOK) {
        SYLAR_LOG_ERROR(g_rpclogger) << "failed to get data of znode: " << path.c_str();
        return "";
    }
    return buffer;
}
