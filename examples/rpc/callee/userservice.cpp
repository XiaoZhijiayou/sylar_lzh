...
public:
    ...
    bool Login(const std::string& name, const std::string& pwd) {
        SYLAR_LOG_INFO(g_logger) << "doing local sevice: Login";
        SYLAR_LOG_INFO(g_logger) << "name: " << name;
        SYLAR_LOG_INFO(g_logger) << "pwd: " << pwd;
        return true;
    }
...
void run() {
    ...
    sylar::rpc::RpcProvider* provider = new sylar::rpc::RpcProvider;
